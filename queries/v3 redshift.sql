...
CombinedResources as (
	...
	FROM tableau_metadata.workbooks ws
	group by ws.name, content_id, id_url, content_type
	union all
	SELECT 
		ds.name, 
		ds.id as content_id,
        -- some values of _datasources.id may are associated to multiple values of data_connections.id
		max(dcs.id) as id_url, 
		'datasources' as content_type
	FROM tableau_metadata.datasources ds
    -- the id of the url is not the id of the table datasources
    -- unfortunately it's the id of the table data_connections
	inner join tableau_metadata.data_connections dcs on ds.id = dcs.datasource_id 
...
PriorityAdjustments AS (
    select
        ...
        END AS new_priority
    FROM
        tableau_metadata.background_jobs
    GROUP BY
        task_id
),
...
CombinedPriorities AS (
    ...
    FROM
        tasks ts
    INNER JOIN tableau_metadata.schedules sc ON ts.schedule_id = sc.id
    INNER JOIN tableau_metadata._sites ss ON ts.site_id = ss.id
